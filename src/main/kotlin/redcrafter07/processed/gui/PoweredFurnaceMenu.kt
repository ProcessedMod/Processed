...
class PoweredFurnaceMenu(
...
) : ProcessedMachineMenu<PoweredFurnaceBlockEntity>(
    ModMenuTypes.POWERED_FURNACE_MENU.get(), containerId, playerInventory, requireFurnaceBlockEntity(blockEntity)
) {
    companion object {
        fun requireFurnaceBlockEntity(b: BlockEntity?): PoweredFurnaceBlockEntity {
            if (b == null) throw IllegalStateException("no block entity found  :<")
            if (b !is PoweredFurnaceBlockEntity) throw IllegalStateException("non-powered furnace block entity found :<")
            return b
        }
    }
    ...
    init {
        addSlot(SlotItemHandler(this.blockEntity.inputItemHandler, 0, 80, 21))
        addSlot(SlotOutputItemHandler(this.blockEntity.outputItemHandler, 0, 80, 59))
        addDataSlots(data)
    }
    ...
    override fun getProgressBar(offX: Int, offY: Int): ProgressBarWidget =
        ProgressBars.POWERED_FURNACE.create(offX + 85, offY + 40, this::progress)

    override val title: Component
        get() = blockEntity.displayName

    override fun customSlotCount(): Int = 2

    override fun stillValid(player: Player): Boolean =
        ContainerLevelAccess.create(level, blockEntity.blockPos).evaluate { level, pos ->
            if (level.getBlockState(pos).block is PoweredFurnaceBlock) return@evaluate player.distanceToSqr(
                pos.x.toDouble() + 0.5,
                pos.y.toDouble() + 0.5,
                pos.z.toDouble() + 0.5,
            ) <= 64.0
