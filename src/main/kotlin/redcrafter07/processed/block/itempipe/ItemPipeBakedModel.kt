...
class ItemPipeBakedModel(val itemTransforms: ItemTransforms) : IDynamicBakedModel {
    val itemConnected = ItemPipeBlockEntity.Connected()

    override fun getQuads(
        p0: BlockState?, p1: Direction?, p2: RandomSource, modelData: ModelData, p4: RenderType?
    ): List<BakedQuad> {
        val quads = ArrayList<BakedQuad>()
        val connected = modelData.get(ItemPipeBlockEntity.TRANSMITTER_PROPERTY) ?: itemConnected
        val sprite = cableTexture.value

        val s = ItemPipeBlock.START
        val e = ItemPipeBlock.END
