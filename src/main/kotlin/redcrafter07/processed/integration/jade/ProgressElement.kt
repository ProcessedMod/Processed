...
class ProgressElement(val component: Component, progress: Float) : Element() {
    ...
    override fun render(
        ...
        graphics.fill(
            x + 1,
            y + 1,
            x + 1 + (progress * (width - 2).toFloat()).toInt(),
            y + height - 1,
            RenderUtils.PROGRESS_RED
        )
        graphics.drawScrollingString(font, component, x + 3, x + width - 3, y + 3, -1)
    }
}